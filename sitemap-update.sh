...
do
	...
	echo -e "<url><loc>https://3v4l.org/$n</loc><lastmod>$d</lastmod><changefreq>monthly</changefreq></url>"
...
do
	...
	cat $f >> $f.xml
	echo '</urlset>' >> $f.xml
	echo "<sitemap><loc>http://3v4l.org/$f.xml</loc><lastmod>`date +%Y-%m-%d`</lastmod></sitemap>" >> sitemap.xml
	rm $f
done
